...
from analytics.strike_dist import pnl_by_strike_dist_bucket, pnl_leg_series
...
class StrikeDistanceView(ttk.Frame):
    def __init__(self, parent):
        ...
        ttk.Label(top, text="Strike Distance Pattern", style="Title.TLabel").grid(row=0, column=0, sticky="w")
        ...
        self.cmb_day = ttk.Combobox(flt, width=10, state="readonly", values=self._available_days, textvariable=self.var_day)
        ...
        self.cmb_cp = ttk.Combobox(flt, width=14, state="readonly", values=self._available_cps, textvariable=self.var_cp)
        ...
        self.cmb_und = ttk.Combobox(flt, width=16, state="readonly", values=self._available_unds, textvariable=self.var_und)
        ...
        # Chart left
        self.chart = MplChart(card, title="PnL by |(Strike-Ref)/Ref| buckets")
        ...
        ttk.Label(panel, text="Strike buckets", style="Muted.TLabel").grid(
            row=0, column=0, sticky="w", padx=12, pady=(12, 6)
        )

        self.table = DataTable(panel, ["Bucket", "Total PnL"], height=18)
        self.table.grid(row=1, column=0, sticky="nsew", padx=12, pady=(0, 12))
        # Disable heading-click sorting (make headings inert)
        for col in ["Bucket", "Total PnL"]:
    ...
    def _rerender(self):
        ...
        if level == "ALL":
            df_sel = df_base[_pair_mask_any(df_base)]
        else:
            df_lvl = apply_row_filters(df, day=day, level=level, cp_sub="", und_sub="")
            if cp != "ALL" and "Counterparty" in df_lvl.columns:
                df_lvl = df_lvl[df_lvl["Counterparty"] == cp]
            if und != "ALL" and "UND_NAME" in df_lvl.columns:
                df_lvl = df_lvl[df_lvl["UND_NAME"] == und]
            df_sel = df_lvl

        total_trades = len(df_base)
        pair_trades = len(df_sel)
        total_pnl = float(pnl_leg_series(df_sel).fillna(0.0).sum())
        ...
        buckets = pnl_by_strike_dist_bucket(df_sel, bucket_pct=2.0)

        # ---- Chart ----
        self.chart.clear()
        ax = self.chart.ax
        ax.set_title("PnL vs |(Strike - Ref) / Ref| (%)")
        ax.set_ylabel("PnL")
        ...
        def _sort_key(lbl: str) -> float:
            try:
                lo = float(str(lbl).split("-")[0].replace("%", "").strip())
                return lo
            except Exception:
                return 1e9

        buckets2 = buckets.copy()
        buckets2["_k"] = buckets2["bucket"].astype(str).map(_sort_key)
        buckets2 = buckets2.sort_values("_k", ascending=True).drop(columns="_k")

        labels = buckets2["bucket"].astype(str).tolist()
        vals = pd.to_numeric(buckets2["pnl"], errors="coerce").fillna(0.0).to_numpy()
        ...
        tbl = tbl.rename(columns={"bucket": "Bucket"})[["Bucket", "Total PnL"]]
