...
def load_data(state: AppState, *, progress: ProgressFn = None) -> None:
    if progress: progress("Generating logs...")
    df = generate_mm_logs(
    ...
    if progress: progress("Cleaning logs...")
...
def run_pairing(state: AppState, *, progress: ProgressFn = None) -> None:
    ...
    if state.filters.enable_L1:
        if progress: progress(f"Running L1 pairing (window={state.filters.l1_window})...")
        res1 = detect_pairs_level1(dfc, time_window=state.filters.l1_window, debug_checks=False)
        df_master = apply_level1_to_master(df_master, res1, prefix="L1")
        state.pairs_L1 = res1["pairs_df"]

    if state.filters.enable_L2:
        if progress: progress(f"Running L2 grouping (window={state.filters.l2_window})...")
        res2 = detect_pairs_level2(dfc, time_window=state.filters.l2_window, debug_checks=False)
        df_master = apply_level2_to_master(df_master, res2, prefix="L2")
        state.groups_L2 = res2["groups_df"]

    if state.filters.enable_L3:
        if progress: progress(f"Running L3 pairing (window={state.filters.l3_window}, delta_qty={state.filters.delta_qty})...")
    ...
    if progress: progress("Pairing done.")
